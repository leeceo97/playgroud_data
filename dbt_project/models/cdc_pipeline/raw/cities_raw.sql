{{ config(
    ...
    merge_update_columns=['CITY_NAME', 'COUNTRY', '_AIRBYTE_AB_ID',
                          '_AIRBYTE_EMITTED_AT', '_AIRBYTE_NORMALIZED_AT',
                          '_AB_CDC_LSN', '_AB_CDC_UPDATED_AT']
) }}

/*
    CDC-based cities_raw table (Incremental)
    - Process only new or updated records from cities_cdc
